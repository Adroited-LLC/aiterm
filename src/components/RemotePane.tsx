...
import {
  PhonePairPayload, PhoneRemoteStatus,
  phoneRemotePairPayload, phoneRemoteRotateToken, phoneRemoteSetEnabled, phoneRemoteSetName, phoneRemoteSetPort, phoneRemoteStatus,
} from "../ipc";
...
export default function RemotePane() {
  ...
  const load = () => phoneRemoteStatus().then((s) => { setStatus(s); if (!editing.current) { setName(s.name); setPort(String(s.port)); } });
  useEffect(() => { load(); }, []);
  ...
  const toggle = async (on: boolean) => {
    const s = await phoneRemoteSetEnabled(on);
    setStatus(s);
    if (!on) setPair(null);
  };
  const showQr = async () => {
    setPairError(null);
    try { setPair(await phoneRemotePairPayload()); }
    catch (e) { setPair(null); setPairError(`${e}`); }
  };
  const forget = async () => {
    setStatus(await phoneRemoteRotateToken());
    setPair(null);
    setConfirmForget(false);
  };
  const commitName = async () => {
    if (!status || name.trim() === status.name) return;
    setStatus(await phoneRemoteSetName(name));
    setPair(null);
  };
  const commitPort = async () => {
    ...
    try { setStatus(await phoneRemoteSetPort(p)); setPortError(null); setPair(null); }
