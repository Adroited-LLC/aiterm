...
export function UpdateNotice() {
  ...
  useEffect(() => {
    let active = true;
    const check = () => { void invoke<Status>("app_update_check").then(s => { if (active) setUpdate(s.available); }).catch(() => {}); };
...
export default function AppUpdates() {
  ...
  useEffect(() => { run(async () => { await check(); }); }, []);
  return <section className="sgroup">
    <h3>App updates</h3>
    <p>{status ? `Installed version ${status.currentVersion}` : "Private updates for AITerm"}</p>
    <p>Use a GitHub token with Contents: read access to Adroited-LLC/aiterm-releases. Your GitHub account must have access to that private repository.</p>
    <label>GitHub access token <input type="password" autoComplete="off" value={token} onChange={e => setToken(e.target.value)} placeholder="Enter token" /></label>
    ...
    <p>Checks run automatically while AITerm is open. Installation always requires your choice.</p>
  </section>;
}
