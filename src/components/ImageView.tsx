...
import { homeAbbrev, openPath } from "../ipc";
...
export default function ImageView({ path, active, refreshKey }: {
  ...
  const [visibleRefresh, setVisibleRefresh] = useState(refreshKey);
  const [failed, setFailed] = useState(false);
  const [openError, setOpenError] = useState<string | null>(null);
  const source = convertFileSrc(path) + `?v=${visibleRefresh}-${revision}`;
  ...
  // Hidden tabs catch up when opened instead of repeatedly decoding in the background.
  useEffect(() => { if (active) setVisibleRefresh(refreshKey); }, [active, refreshKey]);

  useEffect(() => { setFailed(false); setOpenError(null); }, [source]);
  ...
  return <div className="file-view image-view">
    ...
    <div className="image-viewport" ref={viewport} tabIndex={active ? 0 : -1} role="region" aria-label="Image viewer. Use arrow keys or scroll to pan."
      ...
      {failed ? <div className="image-message" role="alert"><strong>Could not display this image</strong><span>The file may be missing, damaged, or use an unsupported image encoding.</span><button className="image-action" onClick={() => setRevision(n => n + 1)}>Try again</button></div> : <div className="image-stage">
        {!ready && <span className="image-loading" role="status">Opening image…</span>}
        <img key={source} src={source} alt={path.split(/[\\/]/).pop() ?? path} draggable={false}
          className="image-canvas" style={{ width: ready ? size.width * scale : undefined, height: ready ? size.height * scale : undefined, visibility: ready ? "visible" : "hidden" }}
          onLoad={e => { setSize({ width: e.currentTarget.naturalWidth, height: e.currentTarget.naturalHeight }); setFailed(false); }}
