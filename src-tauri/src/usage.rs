...
fn curl_get(url: &str, headers: &[String]) -> Result<(u16, String), String> {
...
fn anthropic_source() -> UsageSource {
    ...
    match curl_get("https://api.anthropic.com/api/oauth/usage", &headers) {
...
fn codex_source() -> Option<UsageSource> {
    ...
    Some(
        match curl_get("https://chatgpt.com/backend-api/wham/usage", &headers) {
            Err(e) => UsageSource::failed(
                "codex",
                "Codex",
                "unreachable",
...
fn provider_source(p: &StoredProvider) -> UsageSource {
...
mod tests {
    ...
    fn live_sources_answer() {
        ...
        match std::env::var("OPENROUTER_API_KEY") {
            ...
            Ok(key) => check(provider_source(&StoredProvider {
                id: "openrouter".into(),
