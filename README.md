...
- **Usage.** A strip in the top bar naming each service and its worst number, and a panel behind it with the whole picture: Claude's plan limits and extra-usage credit (the same data as `/usage`), Codex's rate-limit windows and credits if you have it installed, and the credit balance of any API provider in `~/.config/aiterm/providers.json` that publishes one. A service that can't be read says so — a blank row would be indistinguishable from having used nothing.
